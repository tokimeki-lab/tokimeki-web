...
const CalendarNavigation = ({ date }: Props) => {
  ...
  const showLastMonth = lastMonthYYYY > 2015 || (lastMonthYYYY === 2015 && lastMonth.getMonth() > 2)
  ...
  return (
    <div className="flex justify-between py-8 text-sm text-primary font-bold">
      {showLastMonth ? (
        <Link href={`/calendar/${lastMonthYYYY}${lastMonthMM}`} prefetch={false} className="flex gap-1">
      ...
      <Link href={`/calendar/${nextMonthYYYY}${nextMonthMM}`} prefetch={false} className="flex">
